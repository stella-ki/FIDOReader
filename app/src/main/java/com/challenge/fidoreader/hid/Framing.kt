...
import com.challenge.fidoreader.ctap2.bytes
import com.challenge.fidoreader.ctap2.uIntOf
import com.challenge.fidoreader.ctap2.uShortOf
...
import com.challenge.fidoreader.ctap2.Authenticator as Ctap2Authenticator
        ...
        try {
            ...
            when (packet) {
                is Packet.InitPacket -> {
                    Log.d(TAG, "onIntrData : InitPacket")
                    when (packet.cmd) {
                        CtapHidCommand.Init -> {
                ...
                is Packet.ContPacket -> {
                    Log.d(TAG, "onIntrData : ContPacket")
                    if (packet.channelId == BROADCAST_CHANNEL_ID) {
                        // Only INIT messages are allowed on the broadcast channel.
                        throw CtapHidException(CtapHidError.InvalidCid, packet.channelId)
                    }
