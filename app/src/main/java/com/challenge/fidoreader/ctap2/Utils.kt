...
fun Short.bytes(): ByteArray = ByteBuffer.allocate(java.lang.Long.BYTES)
        .putShort(this)
        .array()

@ExperimentalUnsignedTypes
fun UShort.bytes() = ByteBuffer.allocate(java.lang.Long.BYTES)
        .putLong(this.toLong())
        .array()
        .sliceArray((java.lang.Long.BYTES - java.lang.Short.BYTES) until java.lang.Long.BYTES)

@ExperimentalUnsignedTypes
fun UInt.bytes() = ByteBuffer.allocate(java.lang.Long.BYTES)
        .putLong(this.toLong())
        .array()
...
fun ByteArray.byteArrayToHexString(): String? {
    ...
    for (i in this.indices) {
        val tmp = Integer.toHexString(this[i].and(0xFF.toByte()).toInt() ).toUpperCase()
        if (tmp.length == 1) {
            sb.append("0$tmp")
        } else {
            sb.append(tmp)
